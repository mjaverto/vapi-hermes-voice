...
| **Caller spoofing / unwanted callers** | `VHV_ALLOWED_CALLERS` E.164 allowlist on `customer.number` for **inbound** calls; **fails closed** when caller identity is absent (web calls, `metadataSendMode: "off"`). Outbound task calls are not screened against it (there `customer.number` is the callee the operator chose to dial) | Caller-ID spoofing upstream of Vapi is out of scope; the allowlist is a policy filter, not authentication |
...
| **Exposure of the acknowledgement debug surface** (`GET /debug/acks/{call_ref}`) — a read-only endpoint on a service a phone caller's speech reaches | Same `secrets.compare_digest` bearer as `/chat/completions`, checked before anything else; registered behind the same optional `VHV_ROUTE_SECRET` path prefix, so it can never be reachable on a path the chat endpoint 404s; unregistered entirely when `VHV_DEBUG_ACK_JOURNAL=false`; `call_ref` matched against `^([0-9a-f]{12}\|anon-[0-9a-f]{8})$` before it is used as a lookup key, so a caller-supplied path segment never reaches the map or a log line; reads never create entries; 404 (never 403) for unknown, malformed, or aged-out refs | The record contains only phrases the **adapter** chose from `VHV_FILLER_PHRASES`, their channel, and two timestamps — no caller speech, no transcript, no numbers, no secrets — so a reader learns nothing they could not learn by driving a turn with the same key |
| **Unbounded memory in the acknowledgement journal** | Three simultaneous caps, all enforced on write: `max_entries_per_call` (16), `max_calls` (64, LRU), and a per-entry TTL (900 s); phrase text truncated at 200 chars, so the worst case stops depending on operator config. **Worst case ≤ 1024 records, 465 KiB retained** — measured with `tracemalloc` after 160 000 recordings of maximum-length text across 4 000 distinct call refs, and independent of call volume or caller behaviour. Every eviction is counted and reported as `dropped`, so a consumer knows the record is incomplete rather than reading a gap as evidence | An idle process that never sees another request keeps its last (capped) buckets until the next request or read sweeps them |
